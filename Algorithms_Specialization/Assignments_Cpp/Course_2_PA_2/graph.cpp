...
void Graph<T>::Djikstra(const T &source)
{
    ...
    do
    {
        ...
        {
            if (visited_nodes.at(i) == 1)
            { // check if source is in visited cluster
                for (auto &j : vertex_mapping[i])
                { // loop through all edges connected to this node
                    if (visited_nodes.at(j.first) == 0)
                    { // check if dst is in not visited cluster
    ...
    while (!std::all_of(visited_nodes.begin() + 1, visited_nodes.end(), [&](int i)
                        { return i == visited_nodes.at(source); }));
